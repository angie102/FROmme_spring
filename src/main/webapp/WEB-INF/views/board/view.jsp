 				...
 				<div class="board_wrapper">
					...
					<div class="board">
						...
						<ul class="bbsview_list">
							...
							<li class="bbs_date">작성일 : <span>${board.post_date}</span></li>
						...
						<p class="btn_line txt_right">
							<c:choose>
								<c:when test="${(cri.field eq 'board') || (cri.field eq 'reply')}">
								...
								<c:otherwise>
									<a href="/board/list${cri.getListLink()}" class="btn_bbs">목록</a>
									<c:if test="${board.users_id eq session_id}">
										<a href="javascript:deleteBoard()" class="btn_bbs">삭제</a>
										<a href="/board/modify${cri.getListLink()}&post_no=${board.post_no}" class="btn_bbs">수정</a>
									</c:if>
								</c:otherwise>
							</c:choose>
						</p>
						...
						<ul class="near_list mt20">
			                <li><h4 class="prev">다음글</h4>
			                	<c:if test="${!empty nextBoard}">
			                		<c:choose>
			                			<c:when test="${(cri.field eq 'board') || (cri.field eq 'reply')}">
						                	<a href="/board/viewByUser${cri.getListLink()}&post_no=${nextBoard.post_no}">${nextBoard.post_title}</a>
			                			</c:when>
			                			<c:otherwise>
					                		<a href="/board/view${cri.getListLink()}&post_no=${nextBoard.post_no}">${nextBoard.post_title}</a>
			                			</c:otherwise>
			                		</c:choose>
			                	</c:if>
			                	<c:if test="${empty nextBoard}">
			                ...
			                <li><h4 class="next">이전글</h4>
			               		<c:if test="${!empty prevBoard}">
			                		<c:choose>
			                			<c:when test="${(cri.field eq 'board') || (cri.field eq 'reply')}">
						                	<a href="/board/viewByUser${cri.getListLink()}&post_no=${prevBoard.post_no}">${prevBoard.post_title}</a>
			                			</c:when>
			                			<c:otherwise>
					               			<a href="/board/view${cri.getListLink()}&post_no=${prevBoard.post_no}">${prevBoard.post_title}</a>
			                			</c:otherwise>
			                		</c:choose>
			               		</c:if>
			               		<c:if test="${empty prevBoard}">
